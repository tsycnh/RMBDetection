...
2019年8月10日 11:21:55 : step5-15  
所有Dropout改为BN层
transfer learning：30epochs，rmsprop(1e-4)，no aug (step5) ，最优loss\valloss：1.357\8.028 第11个epoch之后出现过拟合
transfer learning：30epochs，rmsprop(1e-3)，no aug (step5) ，最优loss\valloss：2.04\4.70 第10个epoch之后出现过拟合
transfer learning：30epochs，rmsprop(1e-2)，no aug (step5) ，最优loss\valloss：0.585\3.806 未见明显过拟合
transfer learning：30epochs，rmsprop(1e-2)，no aug (step5) ，最优loss\valloss：0.371\3.519 第21个epoch之后出现轻微过拟合
transfer learning：30epochs，rmsprop(1e-2,1e-3,1e-4 每10个epoch改变)，no aug (step5) ，最优loss\valloss：1.316\3.501 第10个epoch之后出现轻微过拟合,后面改学习率基本没啥用
    finetune：位置：activation_46，30epoch，rmsprop（1e-3），no aug(step6)最优loss\valloss：0.0225\0.0196 模型无过拟合,仍有上升空间 模型save：step5_15-6.h5。
    这次的训练效果明显好于之前用Dropout的模型，而且模型快速收敛，仅做了一次finetune实验就达到远超之前的效果。后续可以尝试增加了数据增强的效果，看是否还能保持检测精度。
